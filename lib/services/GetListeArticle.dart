...
class GetListeArticle {
  Scaffold listeArticle(RssSourceModel rssSource) {
    if (rssSource.isparsingsupported == true) {
      switch (rssSource.url) {
        ...
        case 'http://www.opex360.com':
          return Scaffold(
            appBar: AppBar(
              title: Text(rssSource.isparsingsupported.toString()),
              centerTitle: true,
            ),
    ...
    if (rssSource.rss.length != 1) {
      ...
      List<Future<List<ListeArticle>>> ListOfFutureListeArticle =
          List<Future<List<ListeArticle>>>.generate(rssSource.rss.length,
